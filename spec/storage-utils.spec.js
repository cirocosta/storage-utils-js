...
describe("DatabaseTable", function(){
    ...
    describe("When first initializing the db", function(){
        ...
        it("should be able to parse", function(){
            expect(dbTable._parseDatabaseTable()).toBeDefined();
        });
    ...
    describe("If performing CRUD", function(){
        ...
        describe("when fetching all db", function() {
            it("should return empty list if nothing", function() { 
                var all_objs = dbTable.getAll();
                expect(all_objs.length).toEqual(0);
                
                dbTable.putObject(stub_obj);
                all_objs = dbTable.getAll();
                expect(all_objs.length).not.toEqual(0);
            });
        });
